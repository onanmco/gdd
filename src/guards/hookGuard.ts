...
export async function guardHarnessEvent(
  harness: "claude" | "opencode" | "codex" | "other",
  input: unknown,
  ...
  for (const changedPath of paths) {
    const repoRelative = normalizePath(
      isAbsolute(changedPath) ? relative(planRoot, resolve(changedPath)) : changedPath
    );
    ...
    if (!redRecorded && !isTestFile) {
      return {
        allow: false,
        reason:
          `${activeTask} has no red_recorded memory entry yet. Add the planned failing test first.`
      };
    }
  }
...
export function readYamlFromStdin(): unknown {
  const input = readFileSync(0, "utf8").trim();
  if (!input) {
